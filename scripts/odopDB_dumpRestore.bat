...
IF %ERRORLEVEL% NEQ 0 (
  ECHO odopDB_dumpRestore: mysqldump returned ERRORLEVEL %ERRORLEVEL%. Terminating with no attempt to restore.
  GOTO BYEBYE
)
(
...
IF %ERRORLEVEL% NEQ 0 ECHO odopDB_dumpRestore: mysql returned ERRORLEVEL %ERRORLEVEL%
DEL dump_restore_db.txt
ENDLOCAL
GOTO BYEBYE

:NOPARM
ECHO USAGE:  odopDB_dumpRestore type 
...
:ERROUT
ECHO Bad input to odopDB_dumpRestore: %1 %2
ECHO.
