...
import { VALID_MIN, VALID_MAX, MIN, MAX, FDCL } from './actionTypes.js';
import { sclden } from './middleware/sclden.js';
import { initialSystemControls } from '../initialSystemControls.js';
import { logUsage as log } from '../logUsage.js';

export default async function reducers(state = {}, action) {
...
//      console.log('start reducer loadInitialState', 'state=', state, 'action=', action);
      ...
      if (action.payload.units === 'US') {
        moduleName='../designtypes/' + action.payload.type + '/initialState.js';
      } else {
        moduleName='../designtypes/' + action.payload.type + '/initialState_metric_units.js';
      }
      var module = await import(moduleName);
      var module2 = JSON.parse(JSON.stringify(module.initialState)); // Make deep clone
      var result = Object.assign({}, {
        ...state,
...
//        console.log('In reducers.RESTORE_AUTO_SAVE result3=',result);
