...
//    console.log('In init store=',store,'st=',st);
 var i, j;
 var m_tab;
 const ten3 = 1000.0;
 var tensile_400;
...
//console.log("et_tab=", et_tab);
     ...
     st[o.Spring_Type] = "Extension";
     j = st[o.End_Type];
 
 switch(st[o.Prop_Calc_Method]){
 default:
...
//     console.log("case 1 - Use values from material table");
 ...
 //
    i = st[o.Material_Type];
//    st[o.Material_Index] = i;
//    console.log("Material_Index = st[o.Material_Type] =", st[o.Material_Type]);
//    console.log("Material_Index = st[o.Material_Index] =", st[o.Material_Index]);
...
//    astm_fed_spec    = m_tab(i).astm_fs;
    st[o.ASTM_Fed_Spec] = m_tab[i][mo.astm_fs] + '/' + m_tab[i][mo.fedspec];
//    if m_tab(i).kh < 1.0 then process = 'HOT_WOUND';
    if (m_tab[i][mo.kh] < 1.0) {
        st[o.Process] = "Hot_Wound";
    }
//             else process = 'COLD_COILED';
    else {
        st[o.Process] = "Cold_Coiled";
    }
//    density      = m_tab(i).dens;
    st[o.Density]      = m_tab[i][mo.dens];
//    torsion_modulus  = m_tab(i).gg;
    st[o.Torsion_Modulus] = ten3 * m_tab[i][mo.gg];
//
//    hot_factor_kh    = m_tab(i).kh;
    st[o.Hot_Factor_Kh] = m_tab[i][mo.kh];
//    tensile_010      = m_tab(i).t010;
    st[o.tensile_010]   = ten3 * m_tab[i][mo.t010];
//    tensile_400      = m_tab(i).t400;
    tensile_400        = ten3 * m_tab[i][mo.t400];
    
    switch(st[o.Life_Category]){
        default:
        case 1:
        case 5:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte1];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb1];
        break;
        case 2:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte2];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb2];
        break;
        case 3:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte3];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb3];
        break;
        case 4:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte4];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb4];
        break;
        case 6:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte6];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb6];
        break;
        case 7:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte7];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb7];
        break;
        case 8:
            st[o.PC_Tensile_Endur] = m_tab[i][mo.pte8];
            st[o.PC_Tensile_Bend]  = m_tab[i][mo.ptb8];
    }
    
//    pc_tensile_stat  = m_tab(i).fy;
    st[o.PC_Tensile_Stat]  = m_tab[i][mo.pte1];
//    pc_tensile_bend  = m_tab(i).ptb(life_catagory);
//
//    wire_dia=p(2);
//    const_term=log10(tbase010);
    st[o.const_term] = Math.log10(st[o.tbase010]);
//    slope_term=(tensile_400 - tensile_010) /
//           (log10(tbase400) - const_term);
    st[o.slope_term] = (tensile_400 - st[o.tensile_010]) / (Math.log10(st[o.tbase400]) - st[o.const_term]);
...
//  hook_deflect_all=0.0;
    if (st[o.End_Type] <= e_end_num) {
        st[o.Hook_Deflect_All] = et_tab[j][eto.HookDefAll];
    }
...
//        console.log('init: st[o.End_Type] = ', st[o.End_Type]);
...
//    console.log('init: st[o.SI_Range] = ', st[o.SI_Range]);
    if (st[o.SI_Range] <= 2 ) {
        st[o.SI_Lo_Factor] = m_tab[i][mo.silf];
        if (st[o.SI_Range] === 2) {
            st[o.SI_Lo_Factor] = m_tab[i][mo.silf] / 2.0;    // see issue #544
            st[o.SI_Hi_Factor] = m_tab[i][mo.sisr];
        }
        else {
            st[o.SI_Hi_Factor] = m_tab[i][mo.sihf];
        }
    }
...
//     console.log("case 3 - Specify Stress_Lim_Stat & Stress_Lim_Endur");
 ...
 if (si_tab[st[o.SI_Range]][0] === "User_Specified") {
