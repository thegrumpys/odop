import React from 'react';
import { ReportBase } from "./ReportBase" // import the inner non-redux-connected class
import { Row, OverlayTrigger, Tooltip } from 'react-bootstrap';
import * as o from './symbol_table_offsets';
import * as mo from '../mat_ips_offsets';
import { connect } from 'react-redux';

class Report3 extends ReportBase {

    constructor(props) {
        super(props);
//      console.log('In Report3.constructor this=',this,'props=',props);
    }

    render() {
//        console.log('In Report3.render this.props=',this.props);

        return (
            <React.Fragment>
                <h4>ODOP:Spring &nbsp; Compression Spring Report</h4>
                <br />
                <table>
                    <tbody>
                        <tr>
                            <td>{this.labels[o.Contact_person].name}:</td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Contact_person].value}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.labels[o.Phone].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Phone].value}</td>
                        </tr>
                        <tr>
                            <td>{this.labels[o.Company_name].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Company_name].value}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.labels[o.Date].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Date].value}</td>
                        </tr>
                        <tr>
                            <td>{this.labels[o.Street].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Street].value}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.labels[o.Part_Number].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Part_Number].value}</td>
                        </tr>
                        <tr>
                            <td>{this.labels[o.City].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.City].value}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.labels[o.Finish].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.Finish].value}</td>
                        </tr>
                        <tr>
                            <td>{this.labels[o.State___Zip].name}: </td>
                            <td> &nbsp; </td>
                            <td>{this.labels[o.State___Zip].value}</td>
                        </tr>
                    </tbody>
                </table>
                <b>Comment: &nbsp; </b> {this.labels[o.COMMENT].value} <br/>
                <br/>
                <table>
                    <tbody>
                        <tr>
                            <td>{this.st[o.Spring_Type].name}</td>
                            <td>=</td>
                            <td className="" colSpan="2">{this.st[o.Spring_Type].value}</td>
                            <td/>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.Material_Type].name}</td>
                            <td>=</td>
                            <td className="text-left" colSpan="2">{this.matTypeValue}</td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>{this.st[o.Wire_Dia].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Wire_Dia].value.toFixed(4)}</td>
                            <td>{this.st[o.Wire_Dia].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>{this.st[o.ASTM_Fed_Spec].name}</td>
                            <td>=</td>
                            <td className="text-left" colSpan="2">{this.astmFedSpecValue}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>{this.st[o.Mean_Dia].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Mean_Dia].value.toFixed(3)}</td>
                            <td>{this.st[o.Mean_Dia].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>{this.st[o.Tensile].name}</td>
                            <td>=</td>
                            <td>{this.tensileFixed0}</td>
                            <td>{this.st[o.Tensile].units}</td>
                        </tr>
                        <tr>
                            <td>{this.st[o.Spring_Index].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Spring_Index].value.toFixed(3)}</td>
                            <td>{this.st[o.Spring_Index].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>{this.st[o.End_Type].name}</td>
                            <td>=</td>
                            <td className="text-left" colSpan="2">{this.et_tab[this.st[o.End_Type].value][0]}</td>
                        </tr>
                        <tr>
                            <td>{this.st[o.Coils_T].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Coils_T].value.toFixed(3)}</td>
                            <td>{"total " + this.st[o.Coils_T].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>Pitch</td>
                            <td>=</td>
                            <td>{this.pitch.toFixed(3)}</td>
                            <td>{this.st[o.L_Free].units}</td>
                        </tr>
                        <tr>
                            <td>{this.st[o.Coils_A].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Coils_A].value.toFixed(3)}</td>
                            <td>{"active " + this.st[o.Coils_A].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>Helix Angle</td>
                            <td>=</td>
                            <td>{this.hlx_ang.toFixed(2)}</td>
                            <td>degrees</td>
                        </tr>
                        <tr>
                            <td>{this.len_lbl}</td>
                            <td>=</td>
                            <td>{this.wire_len_t.toFixed(3)}</td>
                            <td>{this.st[o.L_Free].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>{this.st[o.Weight].name}</td>
                            <td>=</td>
                            <td>{this.wgt1000.toFixed(3)}</td>
                            <td>{this.st[o.Weight].units}</td>
                            <td>{this.wgt1000_u}</td>
                        </tr>
                        <tr>
                            <td>Safe Load</td>
                            <td>=</td>
                            <td>{this.safe_load.toFixed(3)}</td>
                            <td>{this.safe_load_u}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>{this.st[o.Rate].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Rate].value.toFixed(3)}</td>
                            <td>{this.st[o.Rate].units}</td>
                        </tr>
                        <tr>
                            <td>Safe Travel</td>
                            <td>=</td>
                            <td>{this.safe_travel.toFixed(3)}</td>
                            <td>{this.st[o.L_Free].units}</td>
                            <td/>
                            <td> &nbsp; </td>
                            <td>{this.st[o.Cycle_Life].name}</td>
                            <td>=</td>
                            <td>{this.st[o.Cycle_Life].value.toFixed(0)}</td>
                            <td className="text-left" colSpan="2">{this.cycle_life_u}</td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
                <br/>
                <table>
                    <thead>
                        <tr>
                            <th/>
                            <th/>
                            <th>Free </th>
                            <th>1st Load</th>
                            <th>&nbsp; 2nd Load</th>
                            <td></td>
                            <th> &nbsp; &nbsp; Solid</th>
                            <th></th>
                            <th> &nbsp; &nbsp; </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td><b>Force</b></td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{(0.0).toFixed(2)}</td>
                            <td>{this.st[o.Force_1].value.toFixed(2)}</td>
                            <td>{this.st[o.Force_2].value.toFixed(2)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.Force_Solid].value.toFixed(2)}</td>
                            <td className="text-left" colSpan="2">{this.st[o.Force_1].units}</td>
                            <td/>
                        </tr>
                        <tr>
                            <td><b>Length</b></td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.L_Free].value.toFixed(3)}</td>
                            <td>{this.st[o.L_1].value.toFixed(3)}</td>
                            <td>{this.st[o.L_2].value.toFixed(3)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.L_Solid].value.toFixed(3)}</td>
                            <td>{this.st[o.L_1].units}</td>
                        </tr>
                        <tr>
                            <td><b>Deflection</b></td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{(0.0).toFixed(3)}</td>
                            <td>{this.st[o.Deflect_1].value.toFixed(3)}</td>
                            <td>{this.st[o.Deflect_2].value.toFixed(3)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{(this.st[o.L_Free].value - this.st[o.L_Solid].value).toFixed(3)}</td>
                            <td>{this.st[o.Deflect_2].units}</td>
                        </tr>
                        <tr>
                            <td><b>Outside Dia.</b></td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.OD_Free].value.toFixed(3)}</td>
                            <td>{this.od_1.toFixed(3)}</td>
                            <td>{this.od_2.toFixed(3)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.od_solid.toFixed(3)}</td>
                            <td>{this.st[o.OD_Free].units}</td>
                        </tr>
                        <tr>
                            <td><b>Inside Dia.</b></td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.ID_Free].value.toFixed(3)}</td>
                            <td>{this.id_1.toFixed(3)}</td>
                            <td>{this.id_2.toFixed(3)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{(this.od_solid - 2.0 * this.st[o.Wire_Dia].value).toFixed(3)}</td>
                            <td>{this.st[o.ID_Free].units}</td>
                        </tr>
                        <tr>
                            <td>w/o set &nbsp; kw = </td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.kw1.toFixed(3)}</td>
                            <td></td>
                            <td></td>
                            <td> &nbsp; &nbsp; </td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td> ... <b>Stress</b></td>
                            <td></td>
                            <td></td>
                            <td>{this.st[o.Stress_1].value.toFixed(0)}</td>
                            <td>{this.st[o.Stress_2].value.toFixed(0)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.Stress_Solid].value.toFixed(0)}</td>
                            <td>{this.st[o.Stress_1].units}</td>
                        </tr>
                        <tr>
                            <td> ... <b>% Tensile</b></td>
                            <td></td>
                            <td></td>
                            <td>{(this.st[o.Stress_1].value / this.dhat).toFixed(1)}</td>
                            <td>{(this.st[o.Stress_2].value / this.dhat).toFixed(1)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{(this.st[o.Stress_Solid].value / this.dhat).toFixed(1)}</td>
                            <td>% &nbsp; &nbsp; </td>
                        </tr>
                        <tr>
                            <td> ... <b>Static F.S.</b></td>
                            <td></td>
                            <td></td>
                            <td>{this.fs_1.toFixed(2)}</td>
                            <td>{this.st[o.FS_2].value.toFixed(2)}</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>{this.st[o.FS_Solid].value.toFixed(2)}</td>
                            <td>{this.st[o.FS_Solid].units}</td>
                        </tr>
                    </tbody>
                </table>
                <hr/>
                <table>
                    <tbody>
                        <tr>
                            <td>Wind: </td>
                            <td> &nbsp; </td>
                            <td>rh</td>
                            <td> &nbsp; &nbsp; </td>
                            <td>lh</td>
                            <td> &nbsp; </td>
                            <td>opt</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>Data source: </td>
                            <td> &nbsp; </td>
                            <td>print </td>
                            <td> &nbsp; &nbsp; </td>
                            <td>sample </td>
                            <td> &nbsp; </td>
                            <td>verbal</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>Mandrel: </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; &nbsp; </td>
                            <td>Fits in: </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; </td>
                            <td>Works over:</td>
                        </tr>
                        <tr>
                            <td>Stress relieve</td>
                            <td>/ HT: </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; </td>
                            <td>Squareness</td>
                            <td className="text-left">(deg): </td>
                        </tr>
                        <tr>
                            <td>Shot peen: </td>
                            <td> &nbsp; </td>
                            <td> yes </td>
                            <td> &nbsp; </td>
                            <td> no </td>
                            <td> &nbsp; &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> Operating this.temp: </td>
                        </tr>
                        <tr>
                            <td>Special notes</td>
                            <td className="text-left" colSpan="2">& tolerances: </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; &nbsp; </td>
                            <td> End use: </td>
                            <td> &nbsp; </td>
                        </tr>
                    </tbody>
                </table>
                <hr/>
                Deflection at load point 2 is {this.st[o.PC_Avail_Deflect].value.toFixed(0)}% of total available deflection.<br />
                {this.pcadmsg}{this.pcadmsg !== undefined && <br />}
                {this.errmsg1}{this.errmsg1 !== undefined && <br />}
                {this.errmsg2}{this.errmsg2 !== undefined && <br />}
                {this.errmsg3}{this.errmsg0}
                <hr/>
                <table>
                    <tbody>
                        <tr>
                            <td> &nbsp; approved for mfg.&nbsp; </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> &nbsp; approved for mfg.&nbsp; </td>
                        </tr>
                        <tr>
                            <td> &nbsp; </td>
                        </tr>
                        <tr>
                            <td> by _______________________ &nbsp; </td>
                            <td> &nbsp; date _______ &nbsp; </td>
                            <td> &nbsp; </td>
                            <td> by _______________________ &nbsp; </td>
                            <td> &nbsp; date _______ &nbsp; </td>
                        </tr>
                    </tbody>
                </table>
            </React.Fragment>
        );
    }

}

const mapStateToProps = state => ({
    symbol_table: state.model.symbol_table,
    system_controls: state.model.system_controls,
    labels: state.model.labels,
});

export default connect(mapStateToProps)(Report3);
