...
//    console.log("et_tab=", et_tab);
     ...
     st[o.Spring_Type] = "Compression";
     j = st[o.End_Type];
 
 console.log("Rate_Calc_Method=",st[o.Rate_Calc_Method]);
 switch(st[o.Rate_Calc_Method]){
 default:
 case 1:      // Rate_Calc_Method = 1 - Use Geometry
    store.dispatch(changeSymbolInput("Deflect_1", false));
    store.dispatch(changeSymbolInput("Deflect_2", false));
    break;
 case 2:      // Rate_Calc_Method = 2 - Use Force and Deflection
    store.dispatch(changeSymbolInput("Deflect_1", true));
    store.dispatch(changeSymbolInput("Deflect_2", true));
    break;
 }
...
//  hook_deflect_all=0.0;
    ...
    if(et_tab[j][eto.end_type] !== "User_Specified") {
        st[o.Inactive_Coils] = et_tab[j][eto.inactive_coils];
        st[o.Add_Coils_Solid] = et_tab[j][eto.add_coils_solid];
    }
