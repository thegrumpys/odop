...
//        console.log('In SymbolValueWireDia.onChange event.target.value=',event.target.value);
    ...
    if (auto_fixed) {
      this.props.changeResultTerminationCondition(
        'The value of ' + this.props.element.name + ' has been automatically fixed.')
    }
  }
...
//        console.log('In SymbolValueWireDia.render wire_dia_table=',wire_dia_table);
    var sorted_wire_dia_table = wire_dia_table.sort(function (a, b) {
      return a[0] - b[0]
    }) // sort by value
    ...
    if (sv_icon_alerts.length > 0) {
      sv_value_tooltip =
        <>
          Alerts
          <ul>
            {sv_icon_alerts.map((entry, i) => {
              return <li key={i}>{entry.severity}: {entry.message}</li>
            })}
          </ul>
        </>
    }
      ...
      <>
        ...
        <Modal show={this.state.modal} onHide={this.onClose}>
          <Modal.Header closeButton>
            <Modal.Title>
              {this.props.element.type === 'equationset' &&
              (this.props.element.input ? 'Independent Variable' : 'Dependent Variable')} Value Input
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Table className="border border-secondary" size="sm" style={{ backgroundColor: '#eee' }}>
              ...
              <>
                ...
                <tr>
                  ...
                  <td colSpan="2">
                    <InputGroup>
                      <InputGroup.Radio name="value_input" checked={!this.state.value_input}
                                        onChange={this.onRadio}/><InputGroup.Text>Select std size</InputGroup.Text>
                    </InputGroup>
                    <InputGroup>
                      <InputGroup.Radio name="value_input" checked={this.state.value_input}
                                        onChange={this.onRadio}/><InputGroup.Text>Enter non-std value</InputGroup.Text>
                    </InputGroup>
                  </td>
                  <td></td>
                </tr>
                <tr>
                  ...
                  <td className="align-middle" colSpan="2">
                    <InputGroup>
                      {(this.state.value_input ?
                          <FormControlTypeNumber id={'svwd_' + this.props.element.name} icon_alerts={nvu_icon_alerts}
                                                 className={nvu_value_class} step="any" value={this.props.element.value}
                                                 validmin={this.props.element.validmin}
                                                 validmax={this.props.element.validmax} onChange={this.onChange}/>
                          :
                          <Form.Control as="select" id={'svwd_' + this.props.element.name}
                                        disabled={!this.props.element.input} className={nvu_value_class}
                                        value={default_value === undefined ?
                                          this.props.element.value :
                                          default_value[0]} onChange={this.onSelect}>
                            {sorted_wire_dia_table.map(
                              (value, index) => <option key={index} value={value[0]}>{value[1]}</option>)}
                          </Form.Control>
                      )}
                      <InputGroup.Text>
                  ...
                  <td className={'text-nowrap align-middle small ' +
                  (this.props.system_controls.show_units ? '' : 'd-none')} colSpan="1">{this.props.element.units}</td>
                </tr>
                </tbody>
              </>}
              {this.props.element.type === 'equationset' && !this.props.element.input && !this.props.element.hidden &&
              <>
                ...
                <NameValueUnitsRowDependentVariable key={this.props.element.name} element={this.props.element}
                                                    index={0}/>
              </>}
            ...
            <Table className="border border-secondary" size="sm" style={{ backgroundColor: '#eee' }}>
              ...
              <>
                ...
                <ConstraintsMinRowIndependentVariable key={this.props.element.name} element={this.props.element}
                                                      index={0}/>
              </>}
              {this.props.element.type === 'equationset' && !this.props.element.input && !this.props.element.hidden &&
              <>
                <ConstraintsMinHeaderDependentVariable/>
                <ConstraintsMinRowDependentVariable key={this.props.element.name} element={this.props.element}
                                                    index={0}/>
              </>}
            </Table>
            <Table className="border border-secondary" size="sm" style={{ backgroundColor: '#eee' }}>
              ...
              <>
                ...
                <ConstraintsMaxRowIndependentVariable key={this.props.element.name} element={this.props.element}
                                                      index={0}/>
              </>}
              {this.props.element.type === 'equationset' && !this.props.element.input && !this.props.element.hidden &&
              <>
                <ConstraintsMaxHeaderDependentVariable/>
                <ConstraintsMaxRowDependentVariable key={this.props.element.name} element={this.props.element}
                                                    index={0}/>
              </>}
            </Table>
          </Modal.Body>
