...
//        console.log('In cascade design=',design);
        element.eqns.forEach((entry, index) => {
            if (user_input && entry.eqn === false) { // User Input
                console.log('In cascade USER INPUT','element.name=',element.name,'index=',index,'entry=',entry);
                entry.refs.forEach((ref, ref_index) => {
                    var ref_element = design.model.symbol_table[ref];
                    if (ref_element.lmin & UNINITIALIZED) {
                        console.log('In cascade RESET UNINITIALIZED','element.name=',element.name,'index=',index,'user_input=',user_input,'ref=',ref,'ref_index=',ref_index,'ref_element.name=',ref_element.name);
            ...
            } else if (!user_input && entry.eqn === true) { // Compute
...
//                    store.dispatch(changeSymbolValue(element.name,value));
                    ...
                    console.log('In cascade SKIP EXECUTE - ALL PARMS NOT INITIALIZED','element.name=',element.name,'index=',index);
                }
            } else {
                if (user_input) {
