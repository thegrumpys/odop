...
//    }
    ...
    if (design.system_controls.ioopt > 5) {
        console.log("NumIndFixed=", design.symbol_table.reduce((total, element)=>{return (element.equationset && element.input) && element.lmin&FIXED ? total+1 : total+0}, 0));
        console.log("NumIndFree=", design.symbol_table.reduce((total, element)=>{return (element.equationset && element.input) && element.lmin===0 ? total+1 : total+0}, 0));
    }
    var ncode;
    if(design.symbol_table.reduce((total, element)=>{return (element.equationset && element.input) && element.lmin===0 ? total+1 : total+0}, 0) === 0) {
