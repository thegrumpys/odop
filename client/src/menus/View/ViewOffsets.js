            ...
            <>
                ...
                <Modal show={this.state.modal} onHide={this.toggle}>
                    ...
                    <Modal.Body>
                        ...
                        {Object.entries(this.props.symbol_table).map(([name,element]) => {return (element.type === "equationset" && element.input) ? 'export const ' + name.replace('%','PC').replace(/[^a-zA-Z0-9]/g,'_') + ' = ' + (ip++) + ';\n' : ''})}
                        {'\n// Dependent Variables (input-output)\n'}
                        {Object.entries(this.props.symbol_table).map(([name,element]) => {return ((element.type === "equationset" && !element.input) || (element.type === "calcinput")) ? 'export const ' + name.replace('%','PC').replace(/[^a-zA-Z0-9]/g,'_') + ' = ' + (ix++) + ';\n' : ''})}
                        </pre>
                    </Modal.Body>
                    <Modal.Footer>
