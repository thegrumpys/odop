...
//    console.log('In commonChecks store=',store);
    ...
    for (let i = 0; i < design.model.symbol_table.length; i++) {
        ...
        if (element.format === undefined && typeof element.value === 'number' && element.value <= element.validmin) { 
            ...
            addAlert({ element: element, name: element.name, message: 'INVALID VALUE: ' + element.name + ' (' + element.value.toODOPPrecision() + ') <= ' + validmin, severity: 'Err' });
            ...
            addAlert({ element: element, name: element.name, message: 'INVALID VALUE: ' + element.name + ' (' + element.value.toODOPPrecision() + ') >- ' + validmax, severity: 'Err' });
        }

        if (!element.input && (element.lmin & FIXED && element.vmin > 0.0) && (element.lmax & FIXED && element.vmax > 0.0)) {
            addAlert({ element: element, name: element.name+' MIN', message: 'FIX INCONSISTENT: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value outside the range from '+element.cmin.toODOPPrecision()+' to '+element.cmax.toODOPPrecision(), severity: 'Info' });
            addAlert({ element: element, name: element.name+' MAX', message: 'FIX INCONSISTENT: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value outside the range from '+element.cmin.toODOPPrecision()+' to '+element.cmax.toODOPPrecision(), severity: 'Info' });
        } else if (!element.input && (element.lmin & FIXED && element.vmin > 0.0)) {
            addAlert({ element: element, name: element.name+' MIN', message: 'FIX VIOLATION: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value < '+element.cmin.toODOPPrecision(), severity: 'Info' });
        } else if (!element.input && (element.lmax & FIXED && element.vmax > 0.0)) {
            addAlert({ element: element, name: element.name+' MAX', message: 'FIX VIOLATION: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value > '+element.cmax.toODOPPrecision(), severity: 'Info'  });
        } else if ((element.lmin & CONSTRAINED && element.vmin > 0.0) && (element.lmax & CONSTRAINED && element.vmax > 0.0)) {
            addAlert({ element: element, name: element.name+' MIN', message: 'CONSTRAINT INCONSISTENT: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value outside the range from '+element.cmin.toODOPPrecision()+' to '+element.cmax.toODOPPrecision(), severity: 'Info' });
            addAlert({ element: element, name: element.name+' MAX', message: 'CONSTRAINT INCONSISTENT: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value outside the range from '+element.cmin.toODOPPrecision()+' to '+element.cmax.toODOPPrecision(), severity: 'Info' });
        } else if (element.lmin & CONSTRAINED && element.vmin > 0.0) {
            addAlert({ element: element, name: element.name+' MIN', message: 'CONSTRAINT VIOLATION: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value < '+element.cmin.toODOPPrecision(), severity: 'Info' });
        } else if (element.lmax & CONSTRAINED && element.vmax > 0.0) {
            addAlert({ element: element, name: element.name+' MAX', message: 'CONSTRAINT VIOLATION: ' + element.name + ' (' + element.value.toODOPPrecision() + ') Value > '+element.cmax.toODOPPrecision(), severity: 'Info' });
        }

        if ((element.lmin & FIXED) === 0 && element.cminchoices !== undefined && element.cminchoices.length > 0) {
            addAlert({ element: element, name: element.name+' MIN', message: element.lmin & FDCL ? 'FDCL =' + element.cminchoices[element.cminchoice] : '=' + element.cmin + ' (non-FDCL)', severity: 'Info' });
        }
        if ((element.lmax & FIXED) === 0 && element.cmaxchoices !== undefined && element.cmaxchoices.length > 0) {
            addAlert({ element: element, name: element.name+' MAX', message: element.lmax & FDCL ? 'FDCL =' + element.cmaxchoices[element.cmaxchoice] : '=' + element.cmax + ' (non-FDCL)', severity: 'Info' });
        }
