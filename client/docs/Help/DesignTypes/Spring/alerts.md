...
See also: 
 - [Auto Fix, Auto Search](/docs/Help/autoFixSearchStdSize.html#AutoStdSize)  
...
The value of `Mean_Dia` appears in Report 3 and can be controlled in the Advanced View.  
...
The `FS_CycleLife` variable (Soderberg calculation) remains as a way of gauging cycle life for 
user defined materials (`Prop_Calc_Method` = 2 and 3). 
...
The built-in material table provides allowable stress information derived from published 
s-n (stress versus number of cycles) data 
